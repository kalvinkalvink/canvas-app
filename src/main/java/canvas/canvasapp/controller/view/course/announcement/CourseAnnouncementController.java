...
import canvas.canvasapp.event.task.database.AnnouncementUpdatedEvent;
...
public class CourseAnnouncementController {
	...
	private void initialize() {
		try {
			canvasDataFetchTaskService.startFetchTask(TableFetchType.ANNOUNCEMENT);
		} catch (DataFetchTaskNotSupported e) {
			log.error("Failed to fetch course announcement", e);
		}
		initView();
	}
	...
	private void initView() {
		...
		ObservableList<Node> announcementListViewItems = announcementListView.getItems();
		selectedCourseAnnouncementList
	...
	@EventListener
	public void announcementUpdatedEventListener(AnnouncementUpdatedEvent announcementUpdatedEvent) {
		if(Objects.isNull(courseViewService.getCourse())) return;	// course not selected
		log.debug("Updating course announcement because course table updated");
		initView();
	}
